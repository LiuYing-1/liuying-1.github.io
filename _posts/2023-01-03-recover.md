...
- How did we handle `READ` and `WRITE`? What about page replacement?
  - We have pages sitting on the RAM and pages sitting on the disk and modify and read most of the time hopefully from the RAM and then we need to do these transferred pages and talked about different strategies like LRU to exchange pages. => [Virtual Memory with Paging](https://liuying-1.github.io/advanced-computer-system/abstractions/)
